...
impl Command {
    pub async fn execute(
        ...
        match self {
            ...
            Self::PasteFiles { options } => file_ops::paste(context, *options).await,
            ...
            Self::OpenFile => open_file::open(context, backend).await,
            Self::OpenFileWith { index: None } => {
                open_file::open_with_interactive(context, backend).await
            }
            ...
            Self::RenameFileAppend => {
                rename_file::rename_file_append(context, backend, keymap_t).await
            }
            Self::RenameFileAppendBase => {
                rename_file::rename_file_append_base(context, backend, keymap_t).await
            }
            Self::RenameFilePrepend => {
                rename_file::rename_file_prepend(context, backend, keymap_t).await
            }
            Self::RenameFileKeepExt => {
                rename_file::rename_file_keep_ext(context, backend, keymap_t).await
            }
            ...
            Self::SetMode => set_mode::set_mode(context, backend).await,
            Self::ShowTasks => show_tasks::show_tasks(context, backend, keymap_t).await,
            ...
            Self::NumberedCommand { initial } => {
                numbered_command::numbered_command(context, backend, keymap_t, *initial).await
            }
            ...
            Self::Help => show_help::help_loop(context, backend, keymap_t).await,
            ...
            Self::BookmarkAdd => bookmark::add_bookmark(context, backend).await,
            Self::BookmarkChangeDirectory => {
                bookmark::change_directory_bookmark(context, backend).await
            }
        }
    }
}
